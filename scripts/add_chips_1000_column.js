// Script to add chips_1000 column to tbl_player_expenses
// Run: node scripts/add_chips_1000_column.js

const db = require('../config/database');

async function addChips1000Column() {
  try {
    console.log('🔄 Adding chips_1000 column to tbl_player_expenses...');
    ...
    const checkColumn = await db.queryAll(
      `SELECT COLUMN_NAME 
       ...
       AND TABLE_NAME = 'tbl_player_expenses' 
       AND COLUMN_NAME = 'chips_1000'`
    );

    if (checkColumn && checkColumn.length > 0) {
      console.log('✅ Column chips_1000 already exists in tbl_player_expenses');
      await db.close();
      process.exit(0);
    }
      ...
      db.pool.query(
        ...
        (err, result) => {
          if (err) {
            ...
            console.log('✅ Successfully added chips_1000 column to tbl_player_expenses');
            resolve();
          }
        }
      );
